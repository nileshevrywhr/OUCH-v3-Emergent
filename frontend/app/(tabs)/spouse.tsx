...
export default function SpouseExpensesScreen() {
  ...
  // Fetch expense type data for spouse when component mounts or transactions change
  ...
  // Filter transactions for spouse only
  const spouseTransactions = useMemo(() => {
    return transactions.filter(t => t.user === 'spouse');
  }, [transactions]);

  // Calculate totals for current month (spouse only)
  const monthlyData = useMemo(() => {
    const currentDate = new Date();
    const currentMonth = currentDate.getMonth();
    const currentYear = currentDate.getFullYear();

    const monthlyTransactions = spouseTransactions.filter(t => {
  ...
  // Calculate category-wise spending with sorting (spouse only)
      ...
      >
        ...
        {monthlyData.monthlyTransactions.length > 0 && (
          <View style={styles.section}>
            ...
            <FlatList
              data={monthlyData.monthlyTransactions.slice(0, 10)}
        ...
        {/* Empty State */}
        {spouseTransactions.length === 0 && (
          <View style={styles.emptyState}>
            <Ionicons name="person-circle-outline" size={64} color="#ccc" />
            <Text style={styles.emptyText}>No Spouse Expenses Yet</Text>
            <Text style={styles.emptySubtext}>
              Add expenses for your spouse using the "Log Expense For" toggle in the Add Expense screen.
            </Text>
          </View>
        )}
